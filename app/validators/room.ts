...
export const createRoomValidator = vine.compile(
  vine.object({
    ...
    roomType: vine.string().trim().minLength(3).maxLength(100),
    capacity: vine.number().positive().min(1).max(20),
    pricePerNight: vine.number().positive().decimal([0, 2]),
    status: vine.enum(['available', 'occupied', 'maintenance']).optional(),
  })
)
...
export const updateRoomValidator = vine.compile(
  vine.object({
    ...
    roomType: vine.string().trim().minLength(3).maxLength(100).optional(),
    capacity: vine.number().positive().min(1).max(20).optional(),
